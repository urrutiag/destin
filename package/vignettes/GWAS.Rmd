...
# Process chromatin accessiblility data for cell-type specific GWAS
...
The output of the intermediate step is a list, length the number of genes.  Each element of the list contains rowData which contains the gene info.  Each element also contains a sparse vector, length the number of cells.

Then the list is combined to create an aggregated summarized experiment where rows are genes by entrez ID
...
The result is a data frame with p-value, foldChange, sd from mean for each cell type by study
```{r}
resultsEWCEList
```
...
```{r}
schizophreniaSNPsFile = file.path(magmaDir, "ckqny.scz2snpres.gz")
