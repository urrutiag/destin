...
For this example we begin at this step 3.1. but skip 3.3 through 3.4 due to large file sizes. We begin again at step 3.4.1. Load rse. Previous steps are described for reference.
...
- select outDir as the directory to print all output 
...
- srcDir is the path to our bioinformatic processing scripts (.../destin/src)
...
srcDir=~/Documents/gitRepos/destin/src
model=mm10
...
- blacklistFile is the path to file containing peaks to exclude: either ENCFF547MET.bed.gz (mouse) or ENCFF001TDO.bed.gz (human)

```{bash eval=FALSE} 
export genomeBowtie=/proj/seq/data/MM10_UCSC/Sequence/Bowtie2Index/genome
export genomeMacs2=mm;
export blacklistFile=~/Documents/gitRepos/destin/data/blacklist/ENCFF547MET.bed.gz
```
...
if you have already installed the github repository, you can install the R package from the local copy
...
- outDir is the home directory for input files
...
Destin uses an unsupervised model-based likelihood to estimate number of clusters. Like WCSSE, the likelihood is non-decreasing with increasing number of clusters.  We fit the model log-likelihood as a linear spline function of number of clusters with a single knot. The knot placement which results in the best fit (lowest sum of squared errors) is selected as number of clusters.
...
clusterResults = destinGrid (rse, sampleName,
                      ...
                      nClusters = nClusters,
                      nCores = nCores)
colData(rse)$cluster = clusterResults$cluster$cluster
```
...
We can visualize the clustering results using t-SNE, using default parameters.  Note that we do not expect tSNE to match up precisely with our clustering results as we do not use tSNE to cluster.  It is for visual purposes only.

Annotation by cell type is optional, as cell type is unknown in practice. 
```{r}
plotClusterTsne(rse, clusterResults, 
